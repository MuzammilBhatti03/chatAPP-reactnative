...
const TopicsScreen = ({ route }) => {
  ...
  const markUnreadCountToZero = (roomid) => {
    setData((prevData) =>
      prevData.map((topic) =>
        topic.title === roomid ? { ...topic, unreadCount: 0 } : topic
      )
    );
  ...
  const UsersScreen = () => {
    ...
    // Function to fetch and add failed messages
 const fetchAndAddFailedMessages = async (senderId, receiverId, userID) => {
  try {
    // Fetch failed messages from the API
    const response = await axios.get(`${ipurl}/api/messages/${senderId}/${receiverId}`);

    // Ensure the response contains the messages array
    const messages = response.data.messages;

    if (!Array.isArray(messages)) {
      console.error("No messages found or the response is not in the expected format");
      return;
    }

    // Filter out the failed messages
    const failedMessages = messages.filter(msg => msg.isFailed);
    console.log("failed messages are ",failedMessages);

    // Iterate over each failed message and add it to the database
    for (const message of failedMessages) {
      const { _id, content, senderID, receiverID, createdAt, isFailed } = message;
      const privateRoom = [senderID, receiverID].sort().join("-");
      // Format the message as per your database schema
      const newMessage = {
        _id: _id, // Assuming _id is unique, otherwise generate a unique ID
        content,
        messageId: _id, // Assuming messageId is the same as _id, modify if needed
        username: senderID, // You can modify this if you have a way to get the username
        createdAt,
        roomid:privateRoom,
        isRead: 0, // Assuming the message is unread initially
      };

      // Log the message to be added (optional)
      // console.log("New message to save in DB:", newMessage);
    ...
    // console.log("Failed messages successfully added to the DB");

  } catch (error) {
    // Handle errors, but don't log 404 errors
    if (error.code !== 404) {
      // console.error("Error fetching and adding failed messages:", error);
    }
  }
};
    ...
    const fetchLastMessages = async () => {
      try {
        ...
        for (const user of connectedUsers) {
          ...
          setuserid(res1.data.user._id);
          const privateRoom = [res1.data.user._id, receiverId].sort().join("-");
          // await fetchAndAddFailedMessages(res1.data.user._id,receiverId,res1.data.user._id);
    ...
    const fetchLastMessageOfEachUser=async ()=>{
      for (const user of connectedUsers) {
        const receiverId = user._id;
        const res1 = await axios.get(`${ipurl}/getuser/${userN.trim()}`);
        await fetchAndAddFailedMessages(res1.data.user._id,receiverId,res1.data.user._id);
              ...
              return (
                <TouchableOpacity
                  ...
                  onPress={async () => {
                    ...
                    // console.log("room id is ",item._id,"and ",userid);
                    navigation.navigate("Chat", {
                      username: userN,
  ...
  return (
    ...
    >
      <Tab.Screen
        ...
        options={{
          headerStyle: {
            backgroundColor: "#333B56", // Header background color
          },
